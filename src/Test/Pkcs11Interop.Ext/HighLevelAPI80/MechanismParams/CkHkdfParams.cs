...
internal class CkHkdfParams : ICkHkdfParams
{
        ...
        byte[]? info)
    {
        ...
        this.lowLevelStruct.ulSaltType = (ulong)saltType;
        ...
        if (saltKey != null)
        {
            this.lowLevelStruct.hSaltKey = (ulong)saltKey.ObjectId;
        }

        if (salt != null)
        {
            this.lowLevelStruct.pSalt = MemoryUtils.MemDup(salt);
            this.lowLevelStruct.ulSaltLen = (ulong)salt.Length;
        }

        if (info != null)
        {
            this.lowLevelStruct.pInfo = MemoryUtils.MemDup(info);
            this.lowLevelStruct.ulInfoLen = (ulong)info.Length;
        }
    }
