...
public class T21_VerifyEcdsa
{
    ...
    [TestMethod]
    public void VerifyEcdsa_WithPermitedAlgorithm_Failed()
    {
        ...
        Pkcs11Exception ex = Assert.ThrowsExactly<Pkcs11Exception>(() => session.Verify(mechanism, privateKey, data, signature, out _));
        Assert.AreEqual(CKR.CKR_KEY_FUNCTION_NOT_PERMITTED, ex.RV);
    }

    [TestMethod]
    public void VerifyEcdsa_ReadonlySession_Success()
    {
        byte[] dataToSign = new byte[412];
        Random.Shared.NextBytes(dataToSign);
        byte[] hash = SHA256.HashData(dataToSign);
