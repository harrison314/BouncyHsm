...
public class T21_VerifyEddsa
{
    ...
    [TestMethod]
    public void VerifyEddsa_Ed25519Sign_Success()
    {
        ...
        session.Verify(mechanism, publicKey, dataToSign, signature, out bool isValud);

        Assert.IsTrue(isValud, "Signature is not valid.");
        ...
        session.Verify(mechanism, publicKey, dataToSign, signature, out isValud);
    ...
    public void VerifyEddsa_WithParams_Success(string curveName, bool pfFlag, string contextData)
    {
        byte[] dataToSign = new byte[64];
        Random.Shared.NextBytes(dataToSign);
