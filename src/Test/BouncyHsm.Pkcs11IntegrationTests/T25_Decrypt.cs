...
public class T25_Decrypt
{
    ...
    [TestMethod]
    public void Decrypt_WithPermitedAlgorithm_Failed()
    {
        byte[] chiperText = new byte[32];
        Random.Shared.NextBytes(chiperText);
        ...
        Pkcs11Exception ex = Assert.ThrowsExactly<Pkcs11Exception>(() => session.Decrypt(encryptMechanism, key, chiperText));
        Assert.AreEqual(CKR.CKR_KEY_FUNCTION_NOT_PERMITTED, ex.RV);
    }

    [TestMethod]
    public void Decrypt_ReadonlySession_Success()
    {
        byte[] plainText = new byte[16];
        Random.Shared.NextBytes(plainText);
        ...
        byte[] decrypted = session.Decrypt(encryptMechanism, key, cipherText);
    ...
    [TestMethod]
    public void Decrypt_NotLoggedIn_Success()
    {
        byte[] plainText = new byte[16];
        Random.Shared.NextBytes(plainText);
