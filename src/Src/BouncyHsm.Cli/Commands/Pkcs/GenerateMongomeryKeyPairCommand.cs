...
internal class GenerateMongomeryKeyPairCommand : AsyncCommand<GenerateMongomeryKeyPairCommand.Settings>
{
    internal sealed class Settings : BaseSettings
    {
        ...
        [CommandArgument(3, "[ElipticCurveName]")]
        [Description("Name or OID of mongomery curve name.")]
        public required string OidOrName
        {
            get;
            init;
        }
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        ...
        await AnsiConsole.Status()
           ...
           {
               await client.GenerateMontgomeryKeyPairAsync(settings.SlotId, new GenerateMontgomeryKeyPairRequestDto()
               {
                   OidOrName = settings.OidOrName,
