...
internal class GenerateSlhDsaKeyPairCommand : AsyncCommand<GenerateSlhDsaKeyPairCommand.Settings>
{
    internal sealed class Settings : BaseSettings
    {
        ...
        public required CK_SLH_DSA_PARAMETER_SET SlhDsaParameter
        {
            get;
            init;
        }
    ...
    public override async Task<int> ExecuteAsync(CommandContext context, Settings settings, CancellationToken cancellationToken)
    {
        ...
        await AnsiConsole.Status()
           ...
           {
               await client.GenerateSlhDsaKeyPairAsync(settings.SlotId, new GenerateSlhDsaKeyPairRequestDto()
               {
                   SlhDsaParameter = settings.SlhDsaParameter,
