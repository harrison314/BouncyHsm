...
internal class SlhDsaHashedWrapperSigner : IWrapperSigner
{
    private readonly CKM mechanism;
    private readonly Ckp_CkSignAdditionalContext? mechanismParams;
    private readonly IDigest digest;
    private readonly ILogger<SlhDsaHashedWrapperSigner> logger;

    public SlhDsaHashedWrapperSigner(CKM mechanism,
        Ckp_CkSignAdditionalContext? mechanismParams,
        IDigest digest,
        ILogger<SlhDsaHashedWrapperSigner> logger)
    {
    ...
    public AuthenticatedSigner IntoSigningSigner(KeyObject keyObject, SecureRandom secureRandom)
    {
        ...
        if (keyObject is SlhDsaPrivateKeyObject slhDsaPrivateKeyObject)
        {
            ...
            ISigner signer = HashSlhDsaSignerFactory.Create(slhDsaPrivateKeyObject.CkaParameterSet,
                isDeterministic,
                this.digest);
        ...
        {
            throw new RpcPkcs11Exception(CKR.CKR_KEY_HANDLE_INVALID, $"Mechanism {this.mechanism} required private SLH-DSA key.");
        }
    }
    ...
    public ISigner IntoValidationSigner(KeyObject keyObject)
    {
        ...
        if (keyObject is SlhDsaPublicKeyObject slhDsaPublickeyObject)
        {
            ...
            bool isDeterministic = this.IsDeterministicRequired();
            ISigner signer = HashSlhDsaSignerFactory.Create(slhDsaPublickeyObject.CkaParameterSet,
               isDeterministic,
               this.digest);
        ...
        {
            throw new RpcPkcs11Exception(CKR.CKR_KEY_HANDLE_INVALID, $"Mechanism {this.mechanism} required public SLH-DSA key.");
        }
    }
