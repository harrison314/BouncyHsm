...
internal class CamelliaBufferedCipherWrapper : ICipherWrapper
{
    ...
    private readonly ILogger<CamelliaBufferedCipherWrapper> logger;

    public CamelliaBufferedCipherWrapper(IBufferedCipher bufferedCipher,
        byte[]? iv,
        bool padZeroForWrap,
        CKM mechanismType,
        ILogger<CamelliaBufferedCipherWrapper> logger)
    {
    ...
    private ICipherParameters CreateCipherParams(BufferedCipherWrapperOperation operation, KeyObject keyObject)
    {
        ...
        if (keyObject is CamelliaKeyObject camelliaKeyObject)
        {
            bool opEnable = operation switch
            {
                BufferedCipherWrapperOperation.CKA_ENCRYPT => camelliaKeyObject.CkaEncrypt,
                BufferedCipherWrapperOperation.CKA_DECRYPT => camelliaKeyObject.CkaDecrypt,
                BufferedCipherWrapperOperation.CKA_WRAP => camelliaKeyObject.CkaWrap,
                BufferedCipherWrapperOperation.CKA_UNWRAP => camelliaKeyObject.CkaUnwrap,
            ...
            if (this.iv == null)
            {
                return new KeyParameter(camelliaKeyObject.GetSecret());
            }
            else
            {
                return new ParametersWithIV(new KeyParameter(camelliaKeyObject.GetSecret()), this.iv);
            }
        }
        else
        {
            throw new RpcPkcs11Exception(CKR.CKR_KEY_HANDLE_INVALID, $"Mechanism {this.mechanismType} required CAMELLIA key.");
        }
    }
}