...
internal class EncryptStateWithStreamChipher : EncryptState
{
    private readonly IStreamCipher streamCipher;

    public EncryptStateWithStreamChipher(IStreamCipher streamCipher, CKM mechanism)
        : base(mechanism)
    {
    ...
    {
        return $"Stream Encrypt state with {this.streamCipher.AlgorithmName} for mechanism {this.mechanism}.";
    }
}