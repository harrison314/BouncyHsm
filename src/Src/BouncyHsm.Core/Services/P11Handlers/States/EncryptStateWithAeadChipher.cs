...
internal class EncryptStateWithAeadChipher : EncryptState
{
    private readonly IAeadCipher aeadCipher;

    public EncryptStateWithAeadChipher(IAeadCipher aeadCipher, CKM mechanism)
        : base(mechanism)
    {
    ...
    {
        return $"Aead Encrypt state with {this.aeadCipher.AlgorithmName} for mechanism {this.mechanism}.";
    }
}