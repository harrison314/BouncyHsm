...
public partial class SignRecoverHandler : IRpcRequestHandler<SignRecoverRequest, SignRecoverEnvelope>
{
    ...
    private readonly ILogger<SignRecoverHandler> logger;

    public SignRecoverHandler(IP11HwServices hwServices, ILoggerFactory loggerFactory, ILogger<SignRecoverHandler> logger)
    {
    ...
    public async Task<SignRecoverEnvelope> Handle(SignRecoverRequest request, CancellationToken cancellationToken)
    {
        ...
        if (request.IsSignaturePtrSet)
        {
            ...
            {
                return new SignRecoverEnvelope()
                {
