...
internal abstract class BufferedCipherDeriveKeyGenerator<TKeyObject> : IDeriveKeyGenerator
    where TKeyObject : SecretKeyObject
{
    protected readonly IBufferedCipher bufferedCipher;
    protected readonly byte[] data;
    protected readonly byte[]? iv;
    protected readonly ILogger logger;
    protected IReadOnlyDictionary<CKA, IAttributeValue>? template;

    public BufferedCipherDeriveKeyGenerator(IBufferedCipher bufferedCipher, byte[] data, byte[]? iv, ILogger logger)
    {
    ...
    protected virtual ICipherParameters CreateCipherParams(KeyObject keyObject)
    {
        this.logger.LogTrace("Entering to CreateCipherParams.");

        if (keyObject is TKeyObject eceptedKeyObject)
        {
            if (!eceptedKeyObject.CkaDerive)
            {
                ...
                throw new RpcPkcs11Exception(CKR.CKR_KEY_FUNCTION_NOT_PERMITTED,
                    "The derive operation is not allowed because object is not authorized to derive key (CKA_DERVIVE must be true).");
            }

            if (this.iv == null)
            {
                return new KeyParameter(eceptedKeyObject.GetSecret());
            }
            else
            {
                return new ParametersWithIV(new KeyParameter(eceptedKeyObject.GetSecret()), this.iv);
            }
        }
        else
        {
            throw new RpcPkcs11Exception(CKR.CKR_KEY_HANDLE_INVALID, $"This mechanism required {typeof(TKeyObject).Name}.");
        }
    }
