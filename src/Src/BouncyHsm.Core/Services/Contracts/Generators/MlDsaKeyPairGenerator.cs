...
internal class MlDsaKeyPairGenerator : IKeyPairGenerator
{
    ...
    private readonly ILogger<MlDsaKeyPairGenerator> logger;

    public MlDsaKeyPairGenerator(ILogger<MlDsaKeyPairGenerator> logger)
    {
        this.logger = logger;
    }
    ...
    public (PublicKeyObject publicKey, PrivateKeyObject privateKey) Generate(SecureRandom secureRandom)
    {
        ...
        MlDsaPublicKeyObject pubKeyObject = this.CreatePublicKey(publicKey, this.publicKeyTemplate);
        MlDsaPrivateKeyObject privKeyObject = this.CreatePrivateKey(privateKey, this.privateKeyTemplate);
    ...
    private (AsymmetricKeyParameter publicKey, AsymmetricKeyParameter privateKey) GenerateInternal(SecureRandom secureRandom, CK_ML_DSA_PARAMETER_SET parameterSet)
    {
        ...
        MLDsaParameters parameterSetObject = MlDsaUtils.GetParametersFromType(parameterSet);
        MLDsaKeyPairGenerator generator = new MLDsaKeyPairGenerator();
        generator.Init(new MLDsaKeyGenerationParameters(secureRandom, parameterSetObject));

        Org.BouncyCastle.Crypto.AsymmetricCipherKeyPair keyPair = generator.GenerateKeyPair();

        this.logger.LogInformation("Generated ML-DSA key pair ({mlDsaName})", parameterSet);
        return (keyPair.Public, keyPair.Private);
    }

    private MlDsaPrivateKeyObject CreatePrivateKey(AsymmetricKeyParameter privateKey, IReadOnlyDictionary<CKA, IAttributeValue> privateKeyTemplate)
    {
    ...
    private MlDsaPublicKeyObject CreatePublicKey(AsymmetricKeyParameter publicKey, IReadOnlyDictionary<CKA, IAttributeValue> publicKeyTemplate)
    {
        this.logger.LogTrace("Entering to CreatePublicKey.");

        MlDsaPublicKeyObject mlDsaPublicKeyObject = new MlDsaPublicKeyObject();
        foreach ((CKA attrType, IAttributeValue attrValue) in publicKeyTemplate)
        {
            ...
            mlDsaPublicKeyObject.SetValue(attrType, attrValue, false);
        }

        mlDsaPublicKeyObject.SetPublicKey(publicKey);

        return mlDsaPublicKeyObject;
    }
