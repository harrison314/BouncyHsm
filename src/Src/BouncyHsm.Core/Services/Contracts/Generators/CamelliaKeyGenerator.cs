...
internal class CamelliaKeyGenerator : IKeyGenerator
{
    private readonly ILogger<CamelliaKeyGenerator> logger;
    private IReadOnlyDictionary<CKA, IAttributeValue>? template;

    public CamelliaKeyGenerator(ILogger<CamelliaKeyGenerator> logger)
    {
        this.logger = logger;
    }
    ...
    public SecretKeyObject Generate(SecureRandom secureRandom)
    {
        ...
        CamelliaKeyObject generalSecretKeyObject = new CamelliaKeyObject();
        ...
        this.logger.LogInformation("Created new CAMELLIA-{keySize} key {keyType} wit lenght {keyLength}.",
            valueLen * 8,
    ...
    private void CheckTemplate(IReadOnlyDictionary<CKA, IAttributeValue> template)
    {
        ...
        if (!CamelliaKeyObject.IsKeySizeValid((int)valueLength))
        {
            throw new RpcPkcs11Exception(CKR.CKR_TEMPLATE_INCONSISTENT,
              $"Attribute {CKA.CKA_VALUE_LEN} is invalid for CAMELLIA key.");
        }
        ...
        if ((CKK)template.GetAttributeUint(CKA.CKA_KEY_TYPE, (uint)CKK.CKK_CAMELLIA) != CKK.CKK_CAMELLIA)
        {
            throw new RpcPkcs11Exception(CKR.CKR_TEMPLATE_INCONSISTENT,
              $"Attribute {CKA.CKA_KEY_TYPE} must by {CKK.CKK_CAMELLIA}.");
        }
    }
}
