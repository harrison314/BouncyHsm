...
public static class StorageObjectFactory
{
    ...
    private static StorageObject CreateCore(IReadOnlyDictionary<CKA, IAttributeValue> template, IStorageObjectInternalFactory factory)
    {
        ...
        if (classType == CKO.CKO_PRIVATE_KEY)
        {
            ...
            {
                return factory.Create(StorageObjectInternalType.MontgomeryPrivateKeyObject);
            }
            ...
            {
                return factory.Create(StorageObjectInternalType.MlKemPrivateKeyObject);
            }
        }
        ...
        if (classType == CKO.CKO_PUBLIC_KEY)
        {
            ...
            if (keyType == CKK.CKK_EC_MONTGOMERY)
            {
                return factory.Create(StorageObjectInternalType.MontgomeryPublicKeyObject);
            }
            ...
            if (keyType == CKK.CKK_ML_KEM)
            {
                return factory.Create(StorageObjectInternalType.MlKemPublicKeyObject);
            }
        }
    ...
    private static StorageObject CreateSecretCore(IReadOnlyDictionary<CKA, IAttributeValue> template, IStorageObjectInternalFactory factory)
    {
        ...
        StorageObject storageObject = keyType switch
        {
            ...
            CKK.CKK_BLAKE2B_160_HMAC => factory.Create(StorageObjectInternalType.GenericSecretKeyObject),
