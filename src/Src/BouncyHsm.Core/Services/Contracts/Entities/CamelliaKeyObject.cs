...
public sealed class CamelliaKeyObject : SecretKeyObject
{
    ...
    public CamelliaKeyObject()
        : base(CKK.CKK_CAMELLIA, CKM.CKM_CAMELLIA_KEY_GEN)
    {
        this.CkaValue = Array.Empty<byte>();
        this.CkaValueLen = 0;
    }

    public CamelliaKeyObject(StorageObjectMemento memento)
        : base(memento)
    {
    ...
    public override void SetValue(CKA attributeType, IAttributeValue value, bool isUpdating)
    {
        ...
        {
            if (value.Equals((uint)CKK.CKK_CAMELLIA))
            {
                return;
            }
            else
            {
                throw new RpcPkcs11Exception(CKR.CKR_ATTRIBUTE_VALUE_INVALID,
                   $"Attribute {CKA.CKA_KEY_TYPE} is not {CKK.CKK_CAMELLIA}.");
            }
        }
    ...
    {
        return $"{this.GetType().Name} (CAMELLIA-{this.CkaValueLen * 8}): Id={this.Id}";
    }
